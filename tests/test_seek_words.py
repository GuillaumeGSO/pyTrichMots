...
is_search_by_hint
"""
def test_iis_search_by_hint_NoWord():
    # Returns False if lst_car is empty
    assert sw.is_search_by_hint('') == False
    # Exception if no word
    with pytest.raises(TypeError):
        assert sw.is_search_by_hint()


def test_is_search_by_hint_NoHint():
    # Returns True if no hint provided
    assert sw.is_search_by_hint('abc')
    assert sw.is_search_by_hint('abc', [None, None, None])


def test_is_search_by_hint_Hint():
    # Returns True if no hint provided
    assert sw.is_search_by_hint('abc', [None, 'b', ''])
    assert sw.is_search_by_hint('abc', ['', '', 'c'])
...
def test_search_in_file_1_car():
    result = list(sw.search_in_file(nb_car=99, lst_car=['a']))
    # No word contains all letter
    assert len(result) == 0


def test_search_in_file_1_car_rien():
    result = list(sw.search_in_file(nb_car=99, lst_car=['u']))
    # No word contain all letter
    assert len(result) == 0


def test_search_in_file_2_cars():
    result = list(sw.search_in_file(nb_car=99, lst_car=['b', 'a']))
    # No word contain all letter
    assert len(result) == 0


def test_search_in_file_3_cars():
    result = list(sw.search_in_file(nb_car=99, lst_car=['c', 'a', 'b']))
    # One word only contains all letter
    assert len(result) == 1
    assert "bac" in result

def test_search_in_file_7_cars_with_accent():
    result = list(sw.search_in_file(nb_car=99, lst_car=['e', 'e', 'e','i','o','u']))
    # One word only contains all letter
    assert len(result) == 1
...
def test_search_in_file_3_cars_et_plus():
    ...
    result = list(sw.search_in_file(nb_car=99, lst_car=[
                  'a', 'b', 'c', 'd', 'i', 'o', 'p', 'x', 'y', 'z']))
    #We get all words
    assert len(result) == 4
    assert "bac" in result
...
def test_search_in_file_hint_1():
    result = list(sw.search_in_file(nb_car=99, lst_hint=[None, "p", None]))
...
def test_search_in_file_hint_2():
    result = list(sw.search_in_file(nb_car=99, lst_hint=[None, "y"]))
    assert len(result) == 1
    assert "xyz" in result


def test_search_in_file_hint_3():
    result = list(sw.search_in_file(nb_car=99, lst_hint=["a"]))
...
def test_search_in_file_complete():
    ...
    result = list(sw.search_in_file(nb_car=99, lst_car=[
                  'a', 'b', 'c', 'd', 'i', 'o', 'p', 'x', 'y', 'z'], lst_hint=['a']))
    assert len(result) == 1
    assert "api" in result